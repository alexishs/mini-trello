...
def generer_erreur_si_board_inexistant(nom_board: str, dictionnaire_trello: dict) -> None:
...
def generer_erreur_si_colonne_inexistante(nom_colonne: str, dictionnaire_trello: dict) -> None:
    if not nom_colonne in ("a_faire", "en_cours", "termine"):
        raise Exception(f'La colonne "{nom_colonne}" n\'existe pas.')


def ajouter_tache(dictionnaire_trello: dict, nom_board: str, nom_colonne: str, libelle_tache: str) -> None:
